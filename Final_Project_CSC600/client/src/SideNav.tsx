...
function SongsNav({ state, dispatch }: SideNavProps): JSX.Element {
  ...
  */
  ...
  const filterList = (songs: List<any>) => {
    // return songs.map((song: any) => {
    //   if (filterWord === "") {
    //     return true;
    //   } else {
    //     song.get("songTitle").includes(filterWord);
    //   }
    // })
    //arr.push(song.get("songTitle").includes(filterWord))
    const arr = [];
    for (const song of songs) {
        if (filterWord === "") arr.push(song);
        else { //both keyword and word in db need to be lower case to match
            if (song.get("songTitle").toLowerCase().includes(filterWord.toLowerCase())) {
                arr.push(song);
            }
        }
    }
  ...
  return (
    <Section title="Playlist">
      <input type="text" value={filterWord} onChange={e => setFilterWord(e.target.value)}/>
      {filterList(songs).map(song => (
        <div
          key={song.get('id')}
        ...
        >
          ...
          <p>
          {song.get('songTitle')} by {song.get('artist')} by {song.get('album')}
          </p>
        </div>
      ))}
    </Section>
